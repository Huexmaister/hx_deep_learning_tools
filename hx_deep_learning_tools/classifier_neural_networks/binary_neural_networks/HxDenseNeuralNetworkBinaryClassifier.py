from hx_deep_learning_tools.base_model import HxDeepLearningBaseModel
from hx_deep_learning_tools.aux_resources.hx_model_evaluation_and_metrics import EvaluateClassifier, ClassifierMetricsCalculations
import tensorflow as tf
from hx_deep_learning_tools.aux_resources.shap import MlShapTools
from typing import Dict, Literal, List, Any, Tuple, Optional
...
class HxDenseNeuralNetworkBinaryClassifier(HxDeepLearningBaseModel):
    ...
    def __init__(self,
        ...
        :param id_col_name: Nombre de la columna identificadora
        ...
        self._setup_training_callbacks(self.model_save_path)
    ...
    def fit_and_get_model_and_results(self) -> Tuple[Any, np.ndarray, np.ndarray, Dict[str, Any], List]:
        ...
        # Entrenar modelo usando el metodo anterior
        ...
        x_train_probs = x_train_probs[:, 0]
        x_test_probs = x_test_probs[:, 0]


        # Obtener pesos del modelo (importancia de características)
        model_weights = self.get_weights(self.model)

        # Crear diccionario de pesos por característica
        model_weights_dict = {
            col: float(weight) for col, weight in zip(
                self.x_train_df.columns,
                model_weights[0] if model_weights[0] is not None else [0] * len(self.x_train_df.columns)
            )
        }

        # Crear dataframe con resultados de probabilidades
        probs_result_dict = {
        ...
        # Guardar modelo
        self.model.save(os.path.join(self.model_save_path, 'dnn_model.keras'))
    ...
    def execute_shap_analysis(self) -> pd.DataFrame:
        """
        Ejecuta análisis SHAP para el modelo DNN

        Returns:
            DataFrame con resultados SHAP
        """
